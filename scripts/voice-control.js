...
window.ClearViewVoice = {
    ...
    // Reading state
    isReading: false, // NEW: Track reading state
    isPaused: false,
    readingTimeout: null,
    words: [],
    currentWordIndex: 0,
    ...
    init: function () {
        ...
        this.recognition.onerror = (event) => {
            ...
            if (event.error === 'not-allowed') {
                this.announce("Microphone permission was not granted. Voice control cannot start.");
            } else {
                this.announce("Voice control error: " + event.error);
            }
        };
            ...
            if (commands[cmd].some((phrase) => transcript.includes(phrase))) {
                ...
                if (cmd === "continueReading") {
                    if (this.isPaused && this.words.length > 0) {
                        ...
                        // No need to update storage here, readWordByWord will
    ...
    execute: function (command) {
        switch (command) {
            // ... [scroll, zoom, reload cases remain unchanged] ...
            case "scrollDown":
            ...
            case "pauseReading":
                ...
                chrome.storage.local.set({ isReading: false }); // NEW: Update storage
                break;
    ...
    clearWordHighlights: function () {
        document.querySelectorAll(".clearview-word").forEach((w) => {
            ...
            w.style.fontWeight = "";
            w.style.transform = "scale(1)"; // UPDATED
            w.style.transition = "transform 0.1s ease-out"; // UPDATED
        });
    },
    ...
    highlightWord: function (index) {
        ...
        wordSpans[index].style.transform = "scale(1.15)"; // UPDATED: Make text bigger
        wordSpans[index].style.transition = "transform 0.1s ease-in"; // UPDATED: Smooth transition
    ...
    readPage: function () {
        ...
        if (this.words.length === 0) {
            ...
            this.isReading = false; // NEW: Reset state
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    } else if (message.action === "VOICE_TOGGLE_READING") { // UPDATED
